...
public:
    spsc_queue(size_t capacity) : m_buffer(capacity + 1), m_head(0), m_tail(0), cached_head(0), cached_tail(0)
    {
    }
    ...
    inline bool enqueue(const T& item)
    {
        ...
        if (next == cached_head)
        {
            cached_head = m_head.load(std::memory_order_acquire); // 先读
            if (next == cached_head)
                return false;
        }

        m_buffer[tail] = item;
        m_tail.store(next, std::memory_order_release); // 后写
        return true;
    }
    ...
    inline bool dequeue(T& item)
    {
        ...
        if (head == cached_tail)
        {
            cached_tail = m_tail.load(std::memory_order_acquire); // 先读
            if (head == cached_tail)
                return false;
        }
        ...
        m_head.store(next, std::memory_order_release); // 后写
        return true;
    }
